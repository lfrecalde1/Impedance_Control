...
xd = [1.3 + 0.5*sin(0.2*t);...
      0*ones(1, length(t))];
  
xdp = [(0.5*0.2)*cos(0.2*t);...
       0*ones(1, length(t))];

xdpp = [-(0.5*0.2*0.2)*sin(0.2*t);...
        0*ones(1, length(t))];
...
x_enviroment = [1.5*ones(1, length(t)+1);...
                0.0*ones(1, length(t)+1)];
% Enviroment Force
...
if x(1, 1) > x_enviroment(1, 1) 
    F_enviroment(:, 1) = Ke*(x_enviroment(:, 1)-x(:, 1));
end
% if x(2, 1) > x_enviroment(2, 1) 
%     F_enviroment(2, 1) = Ke(2,2)*(x_enviroment(2, 1)-x(2, 1));
% end
...
for k = 1:length(t)
    ...
    if x(1, k+1) > x_enviroment(1, k+1)
        F_enviroment(:, k+1) = Ke*(x_enviroment(:, k+1)-x(:, k+1));
%     elseif  x(2, k+1) > x_enviroment(2, k+1)
%         F_enviroment(2, k+1) = Ke(2,2)*(x_enviroment(2, k+1)-x(2, k+1));
    else
        F_enviroment(:, k+1) = [0;0];
    end
    
end

for k = 1:10:length(t)
    drawpend2_wall(q(:, k), m1, m2, 0.3, l1, l2, x_enviroment(:, k), xd(:,k));
end

save("Data_PD_gravity.mat", "t", "q", "qd", "qdp", "qe", "qep", "u_cartesian", "T_extern","L1", "L2", "xd", "xdp", "x", "xp", "xe", "xep", "F_enviroment", "x_enviroment")